    ...
    if request.method == "POST":
        if request.files:
            ...
            if isallowed(csvfile.filename):
                ...
                csvfile.save(os.path.join('C:\\Users\\5.413U14M4N\\Desktop\\MAD\\capstone\\flashkard\\static\\',csvfile.filename))
                d=Deck(deck_name=csvfile.filename.split('.')[0], user=current_user.username)
                db.session.add(d)
                db.session.commit()
                with open(f'C:\\Users\\5.413U14M4N\\Desktop\\MAD\\capstone\\flashkard\\static\\{csvfile.filename}', 'r') as f:
                    c=csv.reader(f)
                    next(c)
                    for i in c:
                        print(i)
...
def export(deck):
    ...
    with open(f'{deck}.csv', 'w', encoding="utf-8") as f:
        ...
        for card in c:
            ...
            i=i+1
    time.sleep(2)
    return send_file(f'C:\\Users\\5.413U14M4N\\Desktop\\MAD\\capstone\{deck}.csv', as_attachment=True)
